...
@cloup.option("-d", "--data-object", default=None, required=False, help="Data Object's path")
@cloup.option(
    "-l", "--full-path", is_flag=True, required=False, help="List Data Object full path. It works only for Collections."
)
@click.pass_context
def list_metadata(ctx: click.Context, collection, data_object, full_path) -> None:
    """
    ...
    Args:
        ...
        data_object: path for the data object.
    ...
    with embedded_mongo(ctx):
        ...
        with get_mec(uri=mongo_uri):
            ...
            if collection:
                ...
                return
            if data_object:
                dojs = PosixDataObject(uri=mongo_uri).query(path=data_object)

                if not dojs:
                    print(f"Data object '{data_object}' not found.")
                    return
                for pdo in dojs:
